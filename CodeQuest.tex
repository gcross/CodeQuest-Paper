...
\begin{definition}
Given an operator $O\in \mathcal{P}_N$ --- that is, an operator $O:=O_1\otimes O_2 \otimes O_2 \otimes \cdots \otimes O_n$, where $O_1,\dots,O_n\in\mathcal{P}$ are single-qubit Pauli operators --- and an integer $i$ such that $1\leq i \leq N$, we say that the \emph{$i$-th sub-operator of $O$} is $O_i$.
\end{definition}
...
\item \label{rech-A-unique-sub} for every operator $O\in\bar I$, there is an integer $1 \leq i\leq k$ such that any product of operators of $\bar I$ which includes $O$ can never have the identity for its $i$-th sub-operator.
\end{enumerate}
...
\begin{corolary}
Suppose that we have been given lists of Pauli operators $\bar D$ and $\bar I$ such that the operators in $\bar I$ are independent and $\bar D \cup \bar I$ generate some Pauli subgroup $\mathcal{S}$.  Then there exists lists of operators $\bar D'$ and $\bar I'$ such that $|\bar D'|<|\bar D|$, the operators in $\bar I'$ are independent, and $\bar D' \cup \bar I'$ also generates $\mathcal{S}$.  First, we 
\end{corolary}
