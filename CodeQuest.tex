...
\item the function $e_{\set S}:\pauligroup\to \centralizer_\pauligroup\paren{\set S}$ is defined such that $e_{\set S}(l)$ is the minimizer of $w$ over the set $\left\{o: o\in \centralizer_\pauligroup\paren{\set S}, \{o,l\}=0\right\}$ --- that is, it gives the undetectable error with respect to $\set S$ acting on $l$ that is of minimum weight;
\item the function $m_{\set S}:\pauligroup\times\pauligroup \to \mathscr{N}$ is defined such that $m_{\set S}(l,l'):=\min \{w\paren{e_{\set S}(l)},w\paren{e_{\set S}(l')}\}$ --- that is, it gives the smaller of the weights of the smallest weight errors acting on respectively $\set L$ and $\set L'$;
...
\item given the sets $\set S$ and $\set G$ in the context of Theorem \ref{theorem-SG}, an \emph{optimal choice of logical qubits} is any ordered sequence of conjugal pairs, $\lst P$, such that
\begin{enumerate}
\item $\set U\paren{\{\lst P_i\}_i}$ satisfies the properties of $\set L$ described in Theorem \ref{theorem-SG}, and \\
\item given any other sequence $\lst P'$ that satisfies the first property, we have that $M(\lst P_i)\ge M(\lst P'_i)$ for $i=1\dots|\lst P|$. \\
\end{enumerate}
\end{itemize}
