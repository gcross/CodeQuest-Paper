...
\begin{description}
\item[$0\le i<n$ Case] Let $i':=i+1$.  If there is no operator in $\bar D$ that anti-commutes with $X_{i'}$, then we are done with $\bar D':=\bar D$ and $\bar I':=\bar I$.  Otherwise, let $O$ be some member of $\bar D$ which anti-commutes with $X_{i'}$, and let $\bar D':=[f(A)|A\in \bar D]$ and $\bar I':=[f(A)|A\in\bar I]\cup[O]$, where
$$f(A):=\begin{cases}
...
\item[$n\le i<2n$ Case]  The proof takes nearly the same form as the proof given in the previous case by subsituting operators in the $Z_j$ family in the place of operators in the $X_{i'}$ family.  We describe only the differences here.
