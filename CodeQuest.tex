...
Suppose we are given a pair $(a,b)$ and a seqence of $N$ pairs $\lst O$ all conjugal to some set $\set S$, and two sequences of elements from $\{0,1\}$, $\lst l^{(1)}$ and $\lst l^{(2)}$.  Define
$$b':=b\cdot\prod_{o\in \set X_{\lst l}(\lst P)}o,$$
...
$$
and
$$\set S' := \left[\set S\backslash\paren{\{a,b\} \cup \set U(\lst O)}\right]\cup \{a,b'\} \cup \set U(\lst O').$$
Then
\begin{enumerate}
\item the pair $(a,b')$ and the sequence of pairs $\lst O'$ are all conjugal pairs with respect to $\set S'$; and
\item $\genfun\paren{\{a,b'\}\cup \set U(\lst O')}=\genfun\paren{\{a,b\}\cup \set U(\lst O)}$
\end{enumerate}
\end{lemma}
...
Now suppose we have proven this for all sequences of $N-1$ pairs, and we are given a sequence of $N$ pairs.  Invoking the inductive hypothesis, apply this Lemma recursively to the first $N-1$ elements of $\lst O$ and $\lst l$, and let $b''$, $\lst Q''$ and $\set S''$ refer to the objects described in the Lemma (without the extra primes) as applied to these $N-1$ elements.
...
\begin{cases}
b'' & l^{(1)}_N = 0\\
b'' \cdot p_2(\lst O_N)  &  l^{(1)}_N = 1,\\
\end{cases}
\quad
d_1 :=
\begin{cases}
p_1(\lst O_N) & l^{(1)}_N = 0\\
p_1(\lst O_N) \cdot a &  l^{(1)}_N = 1,\\
\end{cases}
$$
and
$$\set R := \left[\set S''\backslash\{a,b'',p_1(\lst O_N),p_2(\lst O_N)\}\right]\cup \{a,c,d_1,p_2(\lst O_N)\}.$$
Note that for either value of $l^{(1)}$ we have that the pairs $(a,c)$ and $(d_1,p_2(\lst O_N))$ are conjugal with respect to $\set R$ and that $\genfun\paren{\{a,c,d_1,p_2(\lst O_N)\}}=\genfun\paren{\{a,b'',p_1(\lst O_N),p_2(\lst O_N)\}}$; for $l^{(1)}=0$ this conclusion is trivial since $b''=c$ and $d_1=p_1(\lst O_N)$, and for $l^{(1)}=1$ we obtain this conclusion by applying Lemma \ref{combining-pairs}.
...
d_2 :=
\begin{cases}
p_2(\lst O_N) & l^{(2)}_N = 0\\
p_2(\lst O_N) \cdot a &  l^{(2)}_N = 1,\\
\end{cases}
$$
and
...
By recursive application of this Proposition, we obtain the objects (without the primes) $\lst P'$, $\set Q'$, $\lst E'$, and $\lst F'$.  Let
$$o :=\min_{x\in\set U(\set Q') \cup \{p_2(\lst P_i'):\lst F_i' \ne I\}} \om_{\set S}(x).$$
There are two cases:  either $o\in\{p_2(\lst P_i'):\lst F_i' \ne I\}$, or $o\notin\{p_2(\lst P_i):\lst F_i' \ne I\}$ and $o\in\set U(\set Q')$.

We start by considering the first case.  Let $k$ be the smallest integer such that $\{o,\lst F_k'\}$, and let $(b,a):=\lst P'_k$.  Place some arbitrary ordering on $\set Q'$ to form the sequence $\lst Q'$, and then let
$$\lst O_i :=
\begin{cases}
\lst P'_{k+i} & 1 \le i \le N-k \\
\lst Q'_{i-N-k} & N-k+1 \le i \le N-k+|\lst Q|, \\
\end{cases}
$$
and
$$\lst l^{(k)}_i :=
