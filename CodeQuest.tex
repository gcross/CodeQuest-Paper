...
First we shall show that $m_{\set S}(\lst P_n)\ge m_{\set S}(\lst P'_n)$.  Let $(a,b):=\lst P'_n$, and observe that since $\genfun\paren{\set U\paren{\{\lst P_i\}_i}}=\genfun\paren{\set U\paren{\{\lst P'_i\}_i}}$ we know that $a$ and $b$ can be written as products of operators from $\set U\paren{\{\lst P_i\}_i}$, i.e., $a=\prod_{a'\in \set A} a'$ and $b=\prod_{b'\in \set B} b'$ where $\set A,\set B\subseteq\set U\paren{\{\lst P_i\}_i}$.  Let $\set C := \set A\, \cup\, \set B$.  If we can show that there is an undetectable error $e$ (with respect to $\set S$) with a weight no greater than $m_{\set S}(\lst P_n)$ that acts only on a single operator $o\in \set C$, then we are done because either $o\in \set A$ or (non-exclusively) $o\in \set B$ and since no other operator in $\set C$ anti-commmutes with $e$ that means that either $\{e,a\}=0$ or $\{e,b\}=0$, and thus $m_{\set S}(\lst P'_n) \le m_{\set S}(\lst P_n)$.  To do this, take the smallest integer $k$ such that, letting $(c,d)\in\lst P_k$, we have that either $c\in \set C$, or (non-exclusively) $d\in \set C$ and $i\in \set F$.  In the first case, we have that $\{c,\lst E_i\}=0$, and since every other operator in $\set C$ is either $d$ or a member of a pair $\lst P_j$ for $j>k$ we know that $\lst E_i$ anti-commutes with only $c$, so we are done with $e:= \lst E_i$ and $o := c$.  In the second case (assuming that the first case did not hold, i.e. $c\notin \set C$), we have that $\{d,f(k)\}$, and recall that by assumption $w(f(k))\le w(\lst E_n)$.  For every other element in $\set C$, we have that either it is a member of a pair $\lst P_j$ for $j>i$, \emph{or} it is the second member of a pair $\lst P_j$ with $j<k$ such that $j\notin \set F$, and in either case by assumption we know that it commutes with $f(k)$.  Thus we are done, setting $e:= f(k)$ and $o:=d$.  This completes the proof that $m_{\set S}(\lst P_n)\ge m_{\set S}(\lst P'_n)$.
