...
\end{enumerate}
\item finally an \emph{optimal choice of qubits stabilized by} $\set S$ is any choice of qubits stabilized by $\set S$, $\lst P$, such that given any other possible choice $\lst P'$ such that $\genfun\paren{\lst P}=\genfun\paren{\lst P'}$, we have that $m_{\set S}(\lst P_i)\ge m_{\set S}(\lst P'_i)$ for every $i=1\dots|\lst P|$.
\end{itemize}
...
\begin{proposition}
Suppose we are given a choice of logical qubits with respect to some set $\set S$, $\lst P$, and also a sequence of Pauli operators $\lst E$ of the same length as $\lst P$ such that for every $i=1\dots|\lst P|$, we have that
\begin{enumerate}
...
\begin{corollary}
Suppose we are given two pairs of operators $(a,b),(c,d)\in\pauligroup\times\pauligroup$ such that $d_{ab}:=m(a,b)$ and $d_{cd}:=m(c,d)$.  Then $w\paren{e_{\set S}(a\cdot b)} \ge \text{min}(d_a,d_b)$.
\end{corollary}
%@nonl
%@-node:gcross.20090511123440.3:Optimal generators
%@-node:gcross.20090423002455.2:Algorithm
