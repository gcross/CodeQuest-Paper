...
Now suppose that we have proven that this proposition holds for sequences of length $N-1$, and have been given sequences of length $N$.  We now need to show that given any other choice of $N$ qubits stabilized by $\set S$, $\lst P'$, such that $(\genfun\circ\set U)(\lst P)=(\genfun\circ\set U)(\lst P')$, we have that $\lst M(\lst P)_i \ge \lst M(\lst P)_i$ for all $1\le 8.  If $m(\lst P'_N)<m(\lst P_N)=\om_{\set S}(\lst E_N)$ then we are done immediately, so assume that $m(\lst P'_N)\ge m(\lst P_N)=\om_{\set S}(\lst E_N)$.

Consider the set $\set O' := \{\lst P'_i:m(\lst P'_i)=\lst M^{(N)}(\lst P')_N \}$, that is the set of conjugal pairs which have the highest-weight minimum-weight undetectable errors of the sequence.   Since $\set O'$ is a set of conjugal pairs, by definition the first member of every pair must anti-commute with the second member.  Thus, since $U(\set O')\subseteq(\genfun \circ \set U)(\lst P)$, for every $O\in\set O'$ we know that there is some $i\in\{1\dots N\}$ and $k\in \{1,2\}$ such that $p_1(\lst P_i)\in (\set G \circ p_k)(O)$ and $p_2(\lst P_i)\in (\set G \circ p_{3-k})(O)$;  that is to say, the only way that the two members can anti-commute is for one to contain the top member of a pair in $\lst P$ and the other to contain a bottom member of the same pair.

Now, for every $o\in \set U(\set O')$ we have by assumption that $\om_{\set S}(o)\ge \om_{\set S}(o)$ (since recall that otherwise $M(\lst P')<M(\lst P)$, and so we are already done), and therefore since $o\in(\genfun\circ \set S)(lst P)$ we conclude from Lemma \ref{combinations-can't-make-things-better} that $w_{\set S}(g)\ge w(\lst E_N)$ for every $g\in\set G(o)$ for every $o\in \set U(\set O')$.  Thus, if $l$ is the index such that $m(\lst P_i)=w(\lst E_N)$ for $i\ge l$, then we can make the even stronger statement that for every $O\in\set O'$ we know that there is some $l \le i le N$ and $k\in \{1,2\}$ such that $p_1(\lst P_i)\in (\set G \circ p_k)(O)$ and $p_2(\lst P_i)\in (\set G \circ p_{3-k})(O)$.



and the the two operators in each pair in $\set O'$ must anti-commute with each other, we conclude that there must be a pair $x\in\set O'$ such that for some $i\in\{1\dots N\}$ and $k\in \{1,2\}$ we have that $p_1(\lst P_i)\in (\set G \circ p_k)(x)$ and $p_2(\lst P_i)\in (\set G \circ p_{3-k})(x)$ --- in other words, for some pair $\lst P_i$ we must have that the generating set of one of the members of $x$ contains one of the members of $\lst P_i$ and the generating set of the other member of $x$ contains the other.  Without loss of generality, assume that $x=\lst P'_n$, since reordering any subsequence of pairs over which the function $m$ is constant does not affect the value $\lst M(\lst P')$.  Furthermore without loss of generality, assume that $k=1$ --- that is, for some $i$ we have that $p_1(\lst P_i)\in(\set G \circ p_1)(x)$ and $p_2(\lst P_i)\in(\set G \circ p_2)(x)$ --- since flipping the members of a pair likewise does not affect the value of $\lst M(O')$.
