...
\begin{definition}
An \emph{error acting on} $\set X$ is a Pauli operator $o\in \pauligroup$ that anti-commutes with some operator in $\set X$\footnote{Of course, such operators are often called by another name, and that is \emph{gates}.  It all depends on whether the action of the operator was desired or not;  at the moment we are obviously assuming that it is undesired.}.  Assuming that we are in the context of Theorem \ref{theorem-SG}, we call this error \emph{detectable} if $o$ anti-commutes with some operator in $\set S$, and \emph{undetectable} otherwise.%  If we are given one of the specific operators, $x\in \set X$, which anti-commutes with the error, then we say that the error \emph{acts on} $L$.
\end{definition}
...
\item the function $e:\pauligroup\to \powerset(\pauligroup)$ is defined such that $e(\set L)$ gives the set of errors acting on $\set L$;
\item the function $m:\powerset(\pauligroup)\to \mathscr{N}$ is defined such that $m(\set L):=\min_{o\in e(\set L)} w(o)$ --- that is, it gives the weight of the smallest weight error acting on $\set L$;
