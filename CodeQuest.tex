...
\begin{lemma}
Suppose we are given a pair $(a,b)$ and a seqence of $N$ pairs $\lst P$ all conjugal to some set $\set S$, and two sequences of elements from $\{0,1\}$, $\lst l^{(1)}$ and $\lst l^{(2)}$.  Define
$$b':=b\cdot\prod_{o\in \{p_{3-k}(\lst P_i) : k \in \{1,2\}, i\in\{1\dots N\}, l^{(k)}_i=1\}}o,$$
$\lst Q_i := (q^{(1)}_i,q^{(2)}_i)$ where
$$q^{(k)}_i :=
\begin{cases}
...
$$
and
$$\set R := \left[\set S''\backslash\{(a,b''),\lst P_N\}\right]\cup \{(a,c),(d_1,p_2(\lst P_N))\}.$$
