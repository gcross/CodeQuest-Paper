...
Let $(a,b):=\lst P'_n$.  Since $\{a,b\}\subseteq\genfun\paren{\set U\paren{\{\lst P'_i\}_i}}=\genfun\paren{\set U\paren{\{\lst P_i\}_i}}$, we know that there exists sets $\set A,\set B\subseteq \set U\paren{\{\lst P_i\}_i}$ such that $a = \prod_{x\in \set A}x$ and $b=\prod_{x\in\set B} x$.  Furthermore, since $a$ and $b$ anti-commute, we know that either $\set A$ or $\set B$ must contain an element that is the first member of a pair in $\lst P$, since if $\set A$ and $\set B$ only contained products of the second members of pairs then $a$ and $b$ would commute.   Without loss of generality, assume that $\set A$ contains an element which is the first member of a pair in $\lst P$.  We can thus apply Lemma \ref{major-lemma} to conclude that $w\paren{e_{\set S}(a)} \le w\paren{e_{\set S}(x)}$ for every $x\in \set A$, which means in particular that $m_{\set S}(\lst P'_n) \le w\paren{e_{\set S}(a)}\le m_{\set S}(\lst P_n)$ since $\set A$ contains at least one element $x$ (i.e., the first member of a pair in $\lst P$) such that $w(x)\le m_{\set S}(\lst P_n)$.

If $m_{\set S}(\lst P'_n) < m_{\set S}(\lst P_n)$ then we are immediately finished with this proof, so let us assume that $m_{\set S}(\lst P'_n) = m_{\set S}(\lst P_n)$.
