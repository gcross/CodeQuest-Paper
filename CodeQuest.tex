...
To simplify the language for describing this process, we employ a common notation and define the operator family $X_i$, which is parameterized over $i$, to refer to the tensor product operators containing $n-1$ single-qubit identity operators and a single-qubit Pauli $X$ operator located in the $i$-th position of the tensor product.  We anagously defined operator families $Y_i$ and $Z_i$ for respectively the Pauli $Y$ and Pauli $Z$ operators.  With this language, we now state the following preposition:
\end{remark}
...
\li             \For $k \gets 1$ \To $|S|$
\li             \Do % k loop
\li                 \kw{if} $\func{anti}(S[k],P[i])$ \kw{then} $S[k] \gets S[k]\cdot o$
                  \End % k loop
\li             append $o$ to $S$
