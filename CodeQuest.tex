...
\begin{theorem}
Suppose we are given a set of $N$ pairs $\set L$ and a set of Pauli operators $\set S$ such that every pair in $\set L$ is conjugal with respect to $\set U(\set L)\cup\set S$.  Then for every $0\le l \le 2N$, there exist
\begin{itemize}
...
\item for every $x\in\set X$ and $o\in\set O$ we have that $[o,x]=0$;
\item and $|\set O|=l$.
\end{enumerate}
\end{theorem}
...
Let $k$ be the integer such that $o=p_2(\lst P'_k)$.  Let $a := o$, $b:=p_1(\lst P'_k)$, $\lst f^{(1)}$ be a sequence of $|\lst P'_i|$ $1$'s, and
$$\lst f^{(2)}_i :=
\begin{cases}
1 & i > k, \{p_j(\lst P'_i),x\} = 0;
0 & \text{otherwise}.
\end{cases}
$$
