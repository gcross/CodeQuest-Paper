...
If $\lst P$ is a choice of $N$ logical qubits stabilized by $\set S$ such that $\{p_1(\lst P_i)\}_i\}$ is an unimprovable set with respect to $\set S$ that extends to $\{p_2(\lst P_i)\}_i\}$, then $\lst P$ is an optimal choice of qubits.
\end{theorem}
...
Take any pair $O:=(a,b)$ such that $\{a,b\}\in\genfun(\set C)$.  Since $a$ and $b$ anti-commute, it must be the case that there is an integer $i$ such that $o:=p_1(\lst P_i)\in \set G_{\set U(\lst P)}(O)$, because if every operator in $\set G_{\set U(\lst P)}(O)$ were the second member of a pair in $\lst P$ then $a$ and $b$ would commute.  Let $c$ be the choice of $a$ or $b$ such that $o\in \set G_{\set U(\lst P)}(c)$.  By Lemma \ref{combinations-can't-make-things-worse} we know that $\om_{\set S}(c)\ge M^{(N)}_{\set S}(\lst P')_k$, and since we also know by the assumptions of this Theorem that $M^{(N)}_{\set S}(\lst P')_k \le \om_{\set S}(c)=\min_{x\in\{p_1(\lst P_j) : 1 \le j \le N, p_1(\lst P_j)\}} \om_{\set S}(x)$, we conclude we must have that $o\in\set A$. 

We have now demonstrated that for every pair $O:=(a,b)$ such that $\{a,b\}\in\set \genfun(\set C)$, we must have $\set G(O)\cup\set A \ne\emptyset$.  Observe that this means that sets $\set C$ and $\set A$ match the descriptions in Proposition \ref{bound-on-recombinations} (letting set $\set Q:=\{\set P_i\}_i$), and thus we see that it is impossible for $|\set C|>|\set A|$, and so we have a contradiction.  We thus conclude that no such choice $\lst P'$ can exist.
\end{proof}
%@-node:gcross.20100125183648.1326:Proof
%@-node:gcross.20100125183648.1308:Theorem: Optimality condition
...
\item and an unimproveable set of operators $\set O$ with respect to $\set S$ that extends to $\set X:=\set U(\set Q) \cup \paren{\set U(\lst P) \backslash \bigcup_l \set U(\lst O^{(l)})}$;
\end{itemize}
such that
\begin{enumerate}
% 1
...
% 4
\item \label{size of O matches arbitrarily chosen integer} and $|\set O|=n$;
% 5
\item \label{operators in X are no worse than the best qubit in P} $\min_{x\in\set X}\om_{\set S}(x)\ge m_{\set S}(\lst P_{|\lst P|})$.
\end{enumerate}
\end{theorem}

\begin{proof}

Proof by induction.  For the base case, let $\lst P'$ be the empty sequence;  then the conclusions are satisfied vacuously.

Now assume that this Proposition has been proven for some integer $n-1$;  let $\lst P'$, $\lst Q'$, $\set O$ and $\set X'$ be the named objects (without the primes) obtained by recursively applying this Proposition to the integer $k-1$  Let $h$ be the minimizer of $w$ over the set $\bigcup_{o\in\set X'} \set e_{\set S}(o)$.  Let $x$ be either $p_2(\lst P_i)$ for the smallest $k$ such that $\{p_2(\lst P_i),h\}=0$, or if there is no such $i$ then let $x$ be any arbitrarily chosen operator from $\set X'$ such that $\{x,h\}=0$.  Finally, let  $d:=\om_{\set S}(x)$.
...
Let $k$ be the integer such that $o=p_2(\lst P'_k)$, $a := o$ and $b:=p_1(\lst P'_k)$.  Let $\set Q:=\set R'$ and $b'$ be the objects named in Corolary \ref{recombination-to-make-commute} given the pair $(a,b)$, the sequences $\lst f^{(l)}$ and the set $\set R=\set Q'$.
...
Now observe we know that the set $\set O'':=\paren{\set O'\backslash\{b\}}\cup\{b''\}$ is an unimprovable set by Lemma X since $b''$ is a product of elements from $\set O'$, as by assumption $p_1(\lst P_i)\in\set O'$ for every $1 \le i \le |\lst P_i|$.  Note that conclusion \ref{any product has undetectable error not acting on X} of this Theorem continues to hold for $\set O''$ as we have simply replaced an element in $\set O'$ with a product of elements from $\set O'$ to form $\set O''$.

Let $\set O:=\set O''\cup\{x\}$.  By Lemma X we know that $\set O$ is an unimprovable set that extends to $\set X'$, since by assumption for every product of elements in $\set O'$ there is an undetectable error that acts on the product but not on $x$.  Now consider any product of elements in $\set O$.  If the product does not contain a factor of $x$, then there exists an undetectable error that acts on it but not on any element in $\set X$ since $\set X$ contains only products of elements from $\set X'$.  If the product contains a factor of $x$ in addition to other elements, then we know that this is still the case since the fact that the error does not act on $x$ means that it must commute with $x$ and hence act on the product containing $x$;  note also that this error must have weight less than or equal to $x$ and so the addition of $x$ to the product cannot change its weight.  Finally, if the product contains only $x$ then we observe that $f$ is an undetectable error of weight $\om_{\set S}(x)$ that by construction commutes with every operator in $\set X$, as every operator in $\set X$ is either an operator from $\set X'$ that commutes with $f$ or the product of $o$ with an operator from $\set X'$ that does not commute.  Thus, property \ref{any product has undetectable error not acting on X} is satisfied.  Also note that since $\set O$ was formed by removing one element from $\set O'$ and adding two, we see that $|\set O|=|\set O'|+1=n-1+1=n$, and so property \ref{size of O matches arbitrarily chosen integer}.
