...
%@+others
%@+node:gcross.20090519160701.3:Construction of sets
...
\begin{proposition}
Suppose that we have been given lists of Pauli operators $\bar I$ and non-empty $\bar D$ and an integer $0 \leq i < 2n$ such that every operator $O\in \bar I$ satisfies at least one of the following properties:
\begin{itemize}
\item there is an index $1\le j\le min(i,n)$ such that out of all of the operators in $\bar D\cup \bar I$, $O$ is the only one whose $i$-th sub-operator anti-commutes with $Z$.
\item there is an index $1\le j\le min(i-n,n)$ such that out of all of the operators in $\bar D\cup \bar I$, $O$ is the only one whose $i$-th sub-operator anti-commutes with $X$.
\end{itemize}
Then there is an integer $i'>i$ and lists $\bar D'$ and $\bar I'$ such that $\bar D'$ is strictly shorter than $\bar D$, the above properties are satisfied (i.e. by substituting $i:=i'$, $\bar D:=\bar D'$ and $\bar I:=\bar I'$) and the Pauli subgroup generated by $\bar D'\cup \bar I'$ is the same as that generated by $\bar D\cup \bar I$.
\end{proposition}

\begin{proof}
There are two possible cases:  either there is an integer $i'<n$
\begin{description}
\item[Case 1] Suppose that $i<n$.  There are two-subcases: either there is an integer $i'\le n$ such that $D$ 
  \begin{description}
    \item[Case 1A]
    \item[Case 1B]
\end{cases}
