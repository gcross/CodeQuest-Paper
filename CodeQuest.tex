...
In order to prove this, we shall need the following lemmas:
...
\begin{lemma}
In the context of Proposition \ref{proposition-optimality-condition}, suppose that we are given an operator $o\in \genfun\paren{\set U\paren{\{\lst P_i\}_i}}$, and let $\set C\subseteq \set U\paren{\{\lst P_i\}_i}$ be the set such that $o=\prod_{c\in \set C} c$.  If there is at least one operator $c\in \set C$ such that $c$ is the first member of a pair in $\lst P$, then $w\paren{e_{\set S}(o)} \le w\paren{e_{\set S}(c)}$ for all $c\in \set C$.
\end{lemma}

\begin{proof}[Proof of Lemma]
Let $c$ be the minimizer of $w \circ e_{\set S}$.  Since there might be several minimizers, and we know that each minimizer must be the first or second member of some pair $\lst P_i$, choose $c$ so that the integer index $i$ on the pair $\lst P_i$ to which $c$ belongs is smallest;  if both operators in $\lst P_i$ meet this criteria, then let $c$ be the first one.

Suppose that $c$ is the second member of the pair and $i\notin\set F$.  Since for every $i$ we have that $w(\lst E_i) \le w(\lst E_n) \le w\paren{e_{\set S}(c)}$ by the assumptions of the Proposition, due to our choice of $c$ this must mean that there is no operator in $\set C$ which anti-commutes with any operator in the sequence $\lst E$, and hence that no operator in $\set C$ is the first member of a pair in $\lst P$, which contradicts an assumption of this Lemma.  Hence, we conclude that either $c$ is the first member of the pair, or that $c$ is the second member of the pair and $i\in\set F$.

Take any operator $x\in \set C$ that anti-commutes with $e_{\set S}(c)$, and let $\lst P_j$ be the pair of which it is a member.  Since $x$ must satisfy $w\paren{ e_{\set S}(x)}\le w\paren{ e_{\set S}(c)}$, we observe that due to our choice of $c$ we cannot have $j < i$.  Furthermore, observe that by our definition of $c$, the conclusion of the previous paragraph, and the assumptions of the Proposition, we cannot have $j > i$ since every operator in a pair $\lst P_k$ such that $k > i$ must commute with $e_{\set S}(c)$.  Thus, we conclude that $i=j$, and due to our choice of $c$ it must furthermore be the case that $c=x$, which means that $c$ is the only operator in $\set C$ that anti-commutes with $e_{\set S}(c)$.  Thus, $o$ must also anti-commute with $e_{\set S}(c)$, and so $w\paren{e_{\set S}(o)} \le w\paren{e_{\set S}(c)}$.  Since $c$ is the minmizer of $w \circ e_{\set S}$, we conclude that $w\paren{e_{\set S}(o)} \le w\paren{e_{\set S}(c)}$ for all $c\in \set C$.
