...
Given conjugal pairs $Q:=(a,b)$ and $R:=(c,d)$ in relation to some set $\set X$ such that $a\ne c$ and $b\ne d$, we have that
\begin{enumerate}
...
If $\lst P$ is a choice of $N$ logical qubits stabilized by $\set S$ such that $\set T:=\{p_1(\lst P_i)\}_i\}$ is an unimprovable set with respect to $\set S$ that extends to $\set U(\lst P)$, then $\lst P$ is an optimal choice of qubits.
\end{theorem}
...
Proof by contradiction.  Let $\lst P'$ be some choice of qubits stabilized by $\set S$ such that $(\genfun\circ\set U)(\lst P)=(\genfun\circ\set U)(\lst P')$ and there exists some integer $k$ such that $M^{(N)}_{\set S}(\lst P')_k > M^{(N)}_{\set S}(\lst P)_k$;  in particular, let $k$ be the smallest such integer, and let $\set C:=\{\lst P'_i : i \ge k\}$.  Let $l$ be the smallest integer such that $M^{(N)}_{\set S}(\lst P)_l\ge M^{(N)}_{\set S}(\lst P')_k$ or $N+1$ if there is no such integer, and $\set A := \{p_1(\lst P_i) : i \ge l\}$; note that since $M^{(N)}(\lst P')_k > M^{(N)}(\lst P)_k$ we must have $l>k$, and hence $|\set C| > |\set A|$.  Note that by Lemmas \ref{recombining extension elements preserves extension} and \ref{taking subsets of extension elements preserves extension} we see that $\set T$ is an unimprovable set with respect to $\set S$ that extends to $\set C$.

Take any pair $O:=(a,b)$ such that $\{a,b\}\in\genfun(\set C)$.  Since $a$ and $b$ anti-commute, it must be the case that $\set G_{\set U(\lst P)}(O)\cap\set T\ne\emptyset$, because if every operator in $\set G_{\set U(\lst P)}(O)$ were the second member of a pair in $\lst P$ then $a$ and $b$ would commute.  Let $c$ be the choice of $a$ or $b$ such that $\set G_{\set U(\lst P)}(c)\cap\set T\ne\emptyset$.  Since $c \in \genfun(\set C)$, by Lemma \ref{combinations-can't-make-things-worse} we know that $M^{(N)}_{\set S}(\lst P')_k \le \om_{\set S}(c)$.  Since $\set T$ extends to $\set C$, we have that $M^{(N)}_{\set S}(\lst P')_k \le \om_{\set S}(c)\le\min_{x\in\set G_{\set U(\lst P)}(O)\cap\set T\}} \om_{\set S}(x)$, which implies that $\om_{\set S}(x)\ge M^{(N)}_{\set S}(\lst P')_k$ for every $x\in\set G_{\set U(\lst P)}(O)\cap\set T$ and hence $\emptyset\ne\set G_{\set U(\lst P)}(O)\cap\set T=\set G_{\set U(\lst P)}(O)\cap\set A$.
...
Suppose we are given a pair $(a,b)$ and a seqence of $N$ pairs $\lst R$ all conjugal to some set $\set S$, and two sequences of elements from $\{0,1\}$, $\lst f^{(1)}$ and $\lst f^{(2)}$.  Define
$$b':=b\cdot\prod_{1 \le i \le N, k \in \{0,1\}, f^{(k)}_i=1}p_{3-k}(\lst R_i),$$
$\lst R'_i := (o'^{(1)}_i,o'^{(2)}_i)$ where
$$o'^{(k)}_i :=
\begin{cases}
p_k(\lst R_i) & l^{(k)}_i = 0\\
p_k(\lst R_i) \cdot a & l^{(k)}_i = 1,
\end{cases}
$$
and
$$\set S' := \left[\set S\backslash\paren{\{a,b\} \cup \set U(\lst R)}\right]\cup \{a,b'\} \cup \set U(\lst R').$$
Then as long as there are no integers $i$ and $k$ such that $l_i^{(k)}=1$, $p_{k}(\lst R_i)=a$ and $p_{3-k}(\lst R_i)=b$, then
\begin{enumerate}
...
Suppose we are given an unimprovable set $\set O$ with respect to $\set S$.  Let $o$ be any element in $\set O$, and $\set Y\subseteq \set O$ such that $o\in y$ and $\min_{y\in\set Y}(y)=\om_{\set S}(o)$.  Let $o':=\prod_{y\in\set Y}y$.  Then $\set O' := \paren{\set O\backslash\{o\}}\cup\{o'}$ is also an unimprovable set with respect to $\set S$, as well as satisfying the property that $\genfun(\set O)=\genfun(\set O')$.
\end{lemma}

\begin{proof}
The only element changed from $\set O$ to $\set O'$ is the replacement of $o$ with a product of elements including $o$;  since every factor besides $o$ in the new product is also contained in $\set O'$, we conclude that we can obtain $o$ from a product of elements in $\set O'$, and so $\genfun(\set O)=\genfun(\set O')$.
...
\begin{proof}
Take any subset $\set X \subseteq \set O\cup\set Q$ such that $\set A' := \set X\cap \set O' \ne \emptyset$.  We need to show that $\om_{\set S}(x)=\min_{y\in\set A'}\om_{\set S}(y)$, where $x := \prod_{y\in\set X} y$.  Let $\set B := \set X \backslash \set A'$, $a := \prod_{y\in\set A'} y$, $b := \prod_{y\in\set B} y$ and $\set A$ be the subset of $\set O$ such that $\prod_{y\in\set A} y =a$.  Since $\set O$ and $\set O'$ are unimprovable sets, we know that $\min_{y\in\set A'}\om_{\set S}(y)=\om_{\set S}(a)=\min_{y\in\set A}\om_{\set S}(y)$.  Since $x=a\cdot b$, we see that $x$ is equal to the product of a set of elements $\set C := \set A \cup\set B$ such that $B\subseteq \set Q$ and $\set C\cap\set O=\set A$, and since $\set O$ extends to $\set Q$ we conclude that $\om_{\set S}(x)=\min_{y\in\set A}\om_{\set S}(y)=\min_{y\in\set A'}\om_{\set S}(y)$.  Thus, $\set O'$ extends to $\set Q$.
\end{proof}
%@-node:gcross.20100129120448.1281:Lemma: Recombining elements preserves extension
...
\begin{proof}
Take any subset $\set X' \subseteq \set O\cup\set Q'$ such that $\set A := \set X'\cap \set O \ne \emptyset$.  We need to show that $\om_{\set S}(x)=\min_{y\in\set A}\om_{\set S}(y)$, where $x := \prod_{y\in\set X} y$.  Note that since $\set Q'\subseteq\genfun(\set Q)$, there exists a set $\set B\subseteq \set Q$ such that $x = \prod_{y\in\set A\cup\set B} y$, and so since $\set O$ extends to $\set Q$ we conclude that $\om_{\set S}(x)=\min_{y\in\set A}\om_{\set S}(y)$.
\end{proof}
%@-node:gcross.20100129120448.1283:Lemma: Recombining extension elements preserves extension
%@+node:gcross.20100130142759.1299:Lemma: Taking subsets of extension elements preserves extension
\begin{lemma}
\label{taking subsets of extension elements preserves extension}
If $\set O$ is an unimprovable set with respect to $\set S$ that extends to $\set Q$, and $\set Q'\subset \set Q$, then $\set O$ also extends to $\set Q'$.
\end{lemma}

\begin{proof}
Take any subset $\set X' \subseteq \set O\cup\set Q'$ such that $\set A := \set X'\cap \set O \ne \emptyset$.  Since $\set X' \subseteq \set O\cup\set Q$ and $\set O$ extends to $\set Q$, we have that $\om_{\set S}\paren{\prod_{y\in\set X} y}=\min_{y\in\set A}\om_{\set S}(y)$.
\end{proof}
%@-node:gcross.20100130142759.1299:Lemma: Taking subsets of extension elements preserves extension
...
Now assume that this Proposition has been proven for some integer $n-1$;  let $\lst P'$, $\lst Q'$, $\set O'$ and $\set X'$ be the named objects (without the primes) obtained by recursively applying this Theorem to the integer $n-1$  Let $h$ be the minimizer of $w$ over the set $\bigcup_{o\in\set X'} \set e_{\set S}(o)$.  Let $x$ be either $p_2(\lst P_k)$ for the smallest $k$ such that $\{p_2(\lst P_k),h\}=0$, or if there is no such $k$ then let $x$ be any arbitrarily chosen operator from $\set X'$ such that $\{x,h\}=0$.  Finally, let  $d:=\om_{\set S}(x)$.
...
Let $k$ be the integer such that $x=p_2(\lst P'_k)$.  Let $a := x$, $b:=p_1(\lst P'_k)$, and let $\set Q:=\set R'$ and $b'$ be the objects named in Corolary \ref{recombination-to-make-commute} given the pair $(a,b)$ and the set $\set R=\set Q'$.

Now let $\lst f^{(1)}$ be a sequence of $1$'s repeated $|\lst P'_i|$ times, and
$$\lst f^{(2)}_i :=
\begin{cases}
1 & i > k, p_2(\lst P'_i)\in\set X', \{p_2(\lst P'_i),x\} = 0; \\
0 & \text{otherwise}.
\end{cases}
$$
Let $\lst P:=\lst R'$ and $b''$ be the objects named in Lemma \ref{multiple-recombinations} given the pair $(a,b):=(a,b')$, the sequences $\lst f^{(l)}$ and the sequence $\lst R=\lst P'$.

Now observe we know that the set $\set O'':=\paren{\set O'\backslash\{b\}}\cup\{b''\}$ is an unimprovable set by Lemma \ref{replacing element with product preserves unimprovability} as $b''$ is a product of elements from $\set O'$ since by the inductive hypothesis and specifically property \ref{first members are all in O} we know that $p_1(\lst P_i)\in\set O'$ for every $1 \le i \le |\lst P_i|$.

Let $\set X'':=\set X\cup\{x\}$.  Note that $\set X\subseteq\genfun(\set X')$, $\set X''\subseteq\genfun(\set X')$ and $\genfun(\set O')=\genfun(\set O'')$, and so by Lemmas \ref{recombining elements preserves extension} and \ref{recombining extension elements preserves extension} we know that $\set O''$ extends to $\set X$, $\set X'$ and $\set X''$.  Let $\set O:=\set O''\cup\{x\}$, and observe that $\set O\cup\set X=\set O''\cup\set X''$.  Consider any subset of elements $\set C\subseteq \set O\cup\set X$, such that $\set A := \set C \cap\set O\ne \emptyset$, and let $c:=\prod_{y\in\set C} y$.  In order to show that $\set O$ is an unimprovable set that extends to $\set X$, we need to show that $\om_{\set S}(c)\le\min_{y\in\set A} \om_{\set S}(y)$. Let $\set A'' := \set A\backslash \{x\} \equiv \set C\cap \set O''$.  Since $\set O''$ extends to $\set X''$, we know that $\om_{\set S}(c)\le\min_{y\in\set A''} \om_{\set S}(y)$.  If $\min_{y\in\set A''} \om_{\set S}(y)\le\min_{y\in\set A} \om_{\set S}(y)$ then we are done.  Otherwise, it must be the case that $x\in \set A$ and $\om_{\set S}(x)<\min_{y\in\set A''} \om_{\set S}(y)$; since we know that there exists an undetectable error of weight $\om_{\set S}(x)$ that acts on $x$ but not on any operator in $\set X$ (by the way we constructed $\lst P$ and $\lst Q$) --- namely, $h$ --- and since this same error must not act on any operator in $\set A''$ as $\om_{\set S}(x)<\min_{y\in\set A''} \om_{\set S}(y)$, we conclude that it acts on $c$ and hence $\om_{\set S}(c)=\om_{\set S}(x)\le\min_{y\in\set A} y$.  Thus we conclude that $\set O$ is an unimprovable set that extends to $\set X$.

Now observe that $p_1(\lst P_k)$ is a product of elements from an unimprovable set such that $p_1(\lst P_k')$ has the lowest weight undetectable error;  therefore we conclude that $\om_{\set S}\paren{p_1(\lst P_k)}=\om_{\set S}\paren{p_1(\lst P'_k)}$, and so $m(\lst P_k)=m(\lst P_k')$.  Every other change from $\lst P'$ to $\lst P$ involves replacing an element from $\set X'$ with a product of elements from $\set X'$, and so we conclude that for every resulting operator $o\in\set X \cup \set U(\lst P)$ we have that $\om_{\set S}(o)\ge m_{\set S}(\lst P_{|\lst P|})$.  Thus, $m(\lst P_i)=m(\lst P_i')$ for every $1 \le i \le |\lst P'$, and so $M^{(|\lst P|)}(\lst P)=M^{(|\lst P'|)}(\lst P')$, which means in particular that the sequence $M^{(|\lst P|)}(\lst P)=M^{(|\lst P'|)}(\lst P')$ is ordered.  Furthermore, since $\set X$ contains only products of operators in $\set X'$, therefore by Lemma \ref{combinations-can't-make-things-worse} we conclude that $\min_{o\in\set X}\om_{\set S}(o)\ge \min_{o\in\set X'}\om_{\set S}(o)\ge m_{\set S}(\lst P'_{|\lst P'|})=m_{\set S}(\lst P_{|\lst P|})$.  Thus, property \ref{operators in X are no worse than the best qubit in P} is satisfied.  Also note that $p_1(\lst P_k)\in\set O$, as this was true for $\lst P'$ and the only operator we changed, $b\to b''$, was also updated changing $\set O'\to\set O$;  thus property \ref{first members are all in O} is satisfied.
