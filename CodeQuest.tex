...
If $s$ is the identity operator, then let $\lst S:=\lst S'$ and $p:=p$ and we are done.  Otherwise, we shall now show that there must integers $j\in\{1,\dots,N\}\backslash\{\lst k'_i\}$ and $l\in\{0,1\}$ such that $s$ anti-commutes with $P_{j}^{[l]}$, by demonstrating that if this were not the case then $s$ would have to anti-commute with some element in $\lst S'$, leading to a contradiction.

Assume that $s$ commutes with every operator in the set $\{P_j^{[l]}:\quad j\in\{1,\dots,N\}\backslash\{\lst k'_i\}, l\in\{0,1\}.$  Recalling that $s$ is a member of the Pauli group and thus a tensor product of single-particle Pauli spin matrices, and also that $s$ commutes with every member of the set $\{P_{\lst k'_i}^{[p'(i)]}\}_{i=1\dots n-1}$, we see therefore that $s$ must be a product of elements from this set, and thus in particular that there must be some integer $i=1\dots n-1$ such that $s$ anti-commutes with $P_{\lst k'_i}^{[1-p'(i)]}$.  Let $s'$ be the member of $\lst S'$ which anti-commmutes with $P_{\lst k'_i}^{[p'(i)]}$.  

As previously discussed, by construction $s$ commutes with every operator in $\{P_{k'_i}^{[p'(i)]}\}_{i=1\dots n-1}$.  Observe furthermore that the only way that $\lst S'_i$ can anti-commute with  $\{P_{k'_i}^{[p'(i)]}\}_{i=1\dots n-1}$ is if it  if there were some $i\in 1\dots n-1$ such that $s$ and $P_{k'_i}^{[1-p'(i)]}$ anti-commute, then 

 for some $\}_{i=1\dots n-1}$ then it would also anti-commute with $\lst S'_i$


To see why, suppose that this were not true.  Since $s$ is not the identity operator and it commutes with all of the operators in the set  this means that it must consist of a product of operators in a non-trivial subset of $\{P_{k'_i}^{[p'(i)]}\}_{i=1\dots n-1}$;  in particular, it must be the case that there exists an integer $i\in 1,\dots,(n-1)$ such that $s$ anti-commutes with $P_{k'_i}^{[1-p'(i)]}$.  However, observe that $k'_i$ is the only integer in the inclusive range $1,\dots, N$ such that $s$ anti-commutes with $P_{k'_i}^{[1-p'(i)]}$ and $\lst S'_i$ anti-commutes with $P_{k'_i}^{[p'(i)]}$.  To repeat this statement in more concrete terms, $\lst k_i'$ is the only integer such that either $\{s,X_{k'_i}\}=0$, $[s,Z_{k'_i}]=0$ and $\{\lst S'_i,Z_{k'_i}\}=0$, or $\{s,Z_{k_i'}\}=0$, $[s,X_{k'_i}]=0$ and $\{\lst S'_i,X_{k'_i}\}=0$.  From this we conclude that $s$ and $\lst S'_i$ must anti-commute, which leads to a contradiction.
