...
Now observe we know that the set $\set O'':=\paren{\set O'\backslash\{b\}}\cup\{b''\}$ is an unimprovable set by Lemma \ref{replacing element with product preserves unimprovability} since $b''$ is a product of elements from $\set O'$, as by the inductive hypothesis and specifically property \ref{first members are all in O} we know that $p_1(\lst P_i)\in\set O'$ for every $1 \le i \le |\lst P_i|$.

Note that $\set X\subseteq\genfun(\set X')$ and $\genfun(\set O')=\genfun(\set O'')$, and so by Lemma \ref{recombining elements preserves extension} and Lemma {recombining extension elements preserves extension} we know that $\set O''$ extends to $\set X'$ and also $\set X$.  Let $\set O:=\set O''\cup\{x\}$.  Consider any product of elements $\set C\subseteq \set O\cup\set X$, and let $c:=\prod_{y\in\set C} y$ and $\set A := \set C\cap\set O$.  In order to show that $\set O$ is an unimprovable set that extends to $\set X$, we need to show that $\om_{\set S}(c)=\min_{y\in\set A}$. \dots
