...
If $m_{\set S}(\lst P_n) > m_{\set S}(\lst P'_n)$ then we are done, so consider the case where $m_{\set S}(\lst P_n) = m_{\set S}(\lst P'_n)$.  Let $\set X:=\{\lst P_i\}_{m(\lst P_i)=m(\lst P_n)}$.  Since $a$ and $b$ (defined in the last paragraph) anti-commute, and the only pairs of operators in $\genfun\paren{\set U\paren{\{\lst P_i\}_i}}$ that anti-commute with each other are the conjugal pairs, we conclude that there must be some integer $l$ such that $c,d\in\set C$ where $(c,d):=\lst P_l$, and also such that one (or more) of $c\notin \set A$, $d\notin \set A$, $c\notin\set B$, and $d\notin\set B$ is true;  the latter condition comes from the fact that if $c,d\in\set A$ and $c,d\in\set B$, then both $a$ and $b$ contain a factor of $c\cdot d$ and this factor does not cause them to anti-commute.  Let us compare $l$ to $k$ (defined in the last paragraph).  
