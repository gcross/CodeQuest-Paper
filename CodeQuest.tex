...
\begin{lemma}
If $\set O$ is an unimprovable set with respect to $\set S$ that extends to $\{q\}\cup\set Q$, and there exists an undetectable error of weight $\om_{\set S}(q)$ that acts on $q$ but not on any operator in $\set Q$, then $\set O\cup\{q\}$ is an unimprovable set with respect to $\set S$ that extends to $\set Q$.
\end{lemma}

\begin{proof}
Take any subset $\set X\subseteq \set O\cup\{q\}\cup\set S$.  We need to show that if $\set X\cap\paren{\set O\cup\set \{q\}}\ne\emptyset$, then $\om_{\set S}\paren{\prod_{x\in \set X} x} \le \min_{x\in\set X}\om_{\set S}(x)$.
\end{proof}
%@nonl
%@-node:gcross.20100204111315.1302:Lemma
