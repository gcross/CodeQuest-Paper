...
A consequence of not requiring independence of the operators in $\bar O$ is that when the lists $\bar S$ and $\bar G$ are constructed, the operators in $\bar S$ are not necessarily independent.  Fortunately, by encoding the operators into bit strings we can incrementally construct a list of independent operators from $\bar S'$.
