...
%@nonl
%@-node:gcross.20091230134131.1288:Single pair rearrangement.
%@+node:gcross.20091226225139.1291:Gaussian elimination
\begin{lemma}
\label{gaussian-elimination-of-logicals}
...
\item and a set $\set A$ such that for any conjugal pair $X\in(\genfun\circ\set U)(\set C)$ we must have that $\paren{(\set G\circ p_1)(X) \cup (\set G\circ p_2)(X)} \cap \set A \ne \emptyset$;
\end{itemize}
then there exists
\begin{itemize}
\item a conjugal pair $Y$ and set of conjugal pairs $\set D$, all with respect to $\{Y\}\cup \set D \cup \set S$, such that $|\set D| = |\set C|-1$ and $(\genfun\circ\set U)(\{Y\}\cup \set D)=(\genfun\circ\set U)(\set C)$;
\item and an element $a\in \set A$ such that $a\in (\set G\circ p_1)(Y)$ but $a\notin (\set G\circ p_2)(Y)$;
\end{itemize}
such that for any conjugal pair $O\in(\genfun\circ\set U)(\set D)$, we must have that $a\notin\paren{(\set G\circ p_1)(O) \cup (\set G\circ p_2)(O)}$ and $\paren{(\set G\circ p_1)(O) \cup (\set G\circ p_2)(O)} \cap \set A\backslash \{a\} \ne \emptyset$.
\end{itemize}
\end{lemma}

\begin{proof}
Proof by induction on the size of $\set C$.  First assume that $\set C=\{Y'\}$, i.e. that it is a singleton set with an element we name $Y'$.  Let $a$ be the element in $\set A$ such that $a\in (\set G\circ p_1)(Y') \cup (\set G\circ p_2)(Y')$.  Applying Lemma \ref{single-pair-rearrangement} letting $o:=a$, $A:=Y'$, and $Y:=B$, we see that we have an operator $Y$ which is a conjugal pair with respect to $\{Y\}\cup\set S$ and also such that $(\genfun\circ\set U)(\{Y\})=(\genfun\circ\set U)(\{Y'\})$.  Let $\set D:=\emptyset$, and we see that the remaining properties hold trivially, so we are done.

Now let us assume that this lemma has been proven for the case where $|\set C|=n-1$, and we are given a set $\set C$ with $n$ elements.  Take any $X''\in\set C$, and apply the lemma to $\set C\backslash \{X''\}$ and $\set A$ to obtain the objects $Y'$, $\set D'$, and $a$ described in the lemma without the primes.  If $a\notin\paren{(\set G\circ p_1)(X) \cup (\set G\circ p_2)(X)}$, then by the assumptions of the Lemma we know that $\paren{(\set G\circ p_1)(X) \cup (\set G\circ p_2)(X)} \cap \set A\backslash \{a\} \ne \emptyset$, so let $Y:=Y'$ and $\set D:=\set D'\cup\{X''\}$, and we are done.
...
\end{proof}
%@+node:gcross.20091230212318.1289:Elimination to create sequence
\begin{corolary}[of Lemma \ref{gaussian-elimination-of-logicals}]
\label{elimination-to-create-sequence}
In the context of Proposition \ref{proposition-optimality-condition}, suppose we are given the sets $\set C$ and $\set A$ described in Lemma \ref{gaussian-elimination-of-logicals};  then there exists a sequence of conjugal pairs, $\lst X$, with respect to $\{\lst X_i\}_i\cup\set S$, and a sequence of distinct operators, $\lst O$, both of length $N:=|\set C|$, such that
\begin{itemize}
\item $(\genfun \circ \set U)(\lst X)=(\genfun \circ \set U)(\set C)$;
\item $\{\lst O_i\}_i\subseteq \set A$; and
\item for every $i=\{1\dots N\}$ we have that $\lst O_i\in(\set G\circ p_1)(\lst X_i)$.
\end{itemize}
\end{corolary}

\begin{proof}
Proof by induction.  If $\set C$ is empty, then the empty sequences trivially satisfy the properties of this Lemma.

Now suppose that we have proven this Lemma for $|\set C|=N-1$, and assume we have been given sets $\set C$ and $\set A$ such that $|\set C|=N$.  Applying Lemma \ref{gaussian-elimination-of-logicals} to $\set C$ and $\set A$ we obtain the conjugal pair $Y$,  the set of conjugal pairs $\set D$, and the element $a$ described in the conclusions of the Lemma.  Apply this Lemma recursively to the set $\set D$ and $\set A\backslash\{a\}$ and we obtain the sequences $\lst X'$ and $\lst O'$ described (without the primes) in this Lemma.  Define
$$\lst X_i :=
\begin{cases}
\lst X'_i & i < N \\
Y & i = N \\
...
\end{cases}.
$$
First, observe that $a\notin \{\lst O'_i\}_i$, so the elements in $\set O$ are distinct.  Next, observe that $a\in \set A$ and $\{\lst O_i'\}_i\subseteq\set A$ by recursive application of this Lemma, and so $\{\lst O_i\}_i\subseteq \set A$.  Furthermore, observe that $(\genfun \circ \set U)(\lst X')=(\genfun \circ \set U)(\set D)$ by the recursive application of this Lemma, and so $(\genfun \circ \set U)(\lst X)=(\genfun \circ \set U)\paren{\{Y\}\cup\{\lst X'\}_i}=(\genfun \circ \set U)(\set D)$ by Lemma \ref{gaussian-elimination-of-logicals}.  Finally, observe that for $i<N$ we have that $\lst O_i\in(\set G\circ p_1)(\lst X_i)$ by recursive application of this Lemma, and for $i=N$ we have that $\lst O_N\in(\set G\circ p_1)(\lst X_N)$ by Proposition \ref{proposition-optimality-condition}.
\end{proof}
%@nonl
%@-node:gcross.20091230212318.1289:Elimination to create sequence
%@+node:gcross.20091230212318.1290:Upper bound on number of pairs
\begin{corolary}[of Lemma \ref{gaussian-elimination-of-logicals}]
In the context of Proposition \ref{proposition-optimality-condition}, there are no sets $\set C$ and $\set A$ as described in Lemma \ref{gaussian-elimination-of-logicals} where $|\set C|>|\set A|$.
\end{corolary}

\begin{proof}
Proof by contradiction.  Applying Corolary \ref{elimination-to-create-sequence}, we see that if such sets existed, then there would have to be a sequence of operators $\lst O$ such that $\{\lst O_i\}_i\subseteq \set A$ and yet $|\lst O|>|\set A|$, which is impossible.
\end{proof}
%@-node:gcross.20091230212318.1290:Upper bound on number of pairs
%@+node:gcross.20091230212318.1291:All are present
\begin{corolary}[of Lemma \ref{gaussian-elimination-of-logicals}]
In the context of Proposition \ref{proposition-optimality-condition}, given $\set C$ and $\set A$ as described in Lemma \ref{gaussian-elimination-of-logicals} where $|\set C|=|\set A|$, for every element $a\in\set A$ there exists a pair $X\in\set C$ such that either $a\in p_1(X)$ or $a\in p_2(X)$.
\end{corolary}

\begin{proof}
By Corolary \ref{elimination-to-create-sequence}, we know that there exists a sequence of conjugal pairs $\lst X$ such that $(\genfun\circ\set U)(\{\lst X_i\}_i)=(\genfun\circ\set U)(\set C)$ and for every $a$ there exists a pair $Y\in \{\lst X_i\}_i$ such that $a\in(\set G\circ p_1)(Y)$.  Thus, we conclude that $\set A \subseteq (\genfun\circ\set U)(\{\lst X_i\}_i)=(\genfun\circ\set U)(\set C)$.
\end{proof}
%@-node:gcross.20091230212318.1291:All are present
%@-node:gcross.20091226225139.1291:Gaussian elimination
%@-node:gcross.20091221145013.1282:Lemmas
