...
%@nonl
%@-node:gcross.20100119183304.1293:Definition: Certificate
%@+node:gcross.20091221145013.1280:Proposition: Certificate = Optimality
%@+node:gcross.20091221145013.1281:Statement
\begin{proposition}
\label{proposition-optimality-condition}
If $\lst P$ has a certificate $(\lst E,\lst F)$, then it is an \emph{optimal choice of qubits stabilized by} $\set S$.
\end{proposition}
%@-node:gcross.20091221145013.1281:Statement
...
In the context of Proposition \ref{proposition-optimality-condition}, if we are given an operator $o\in (\genfun\circ\set U)(\lst P)$ such that $\om_{\set S}(o)\ge w(\lst E_k)$ for some $1\le k \le N$, then $\om_{\set S}(g)\ge w(\lst E_k)$ for every $g\in\set G(o)$.
\end{lemma}

\begin{proof}[Proof of Lemma]
Proof by contradiction.  Let $g\in \set G(o)$ be the operator such that $\om_{\set S}(g)<w(\lst E_k)$, the pair $\lst P_i$ to which $g$ belongs has minimal $i$, and if both operators in $\lst P_i$ meet these conditions then let $g$ be the first one.  Note that since $\om_{\set S}(g)<w(\lst E_k)$, by the assumptions of Proposition \ref{proposition-optimality-condition} we must have that either $g$ is the first member of a pair or $\lst F_i\ne I$.  In either case, we know that there is an operator --- respectively, either $\lst E_i$ or $\lst F_i$ --- which anti-commutes with $g$ but commutes with every other operator in $\set G(o)$.  Thus, the product of all of the operators in $\set G(o)$ must also anti-commute with $\lst E_i$, and thus $o$ anti-commutes with $\lst E_i$.  Since $w(\lst E_i)<w(\lst E_k)$, this contradicts the assumption that $\om_{\set S}(o)\ge w(\lst E_k)$, and so we conclude that all operators $g\in\set G(o)$ must satisfy $\om_{\set S}(g)\ge w(\lst E_k)$.
\end{proof}
%@-node:gcross.20091221145013.1284:Combinations cannot make things better
%@+node:gcross.20100119121615.1280:Each pair must contain a top and a bottom
\begin{lemma}
\label{each-pair-must-contain-a-top-and-a-bottom}
In the context of Proposition \ref{proposition-optimality-condition}, suppose we are given a pair of operators, $a,b\in(\genfun\circ\set U)(\lst P)$, such that $\{a,b\}=0$.  Then there is some $i\in\{1\dots N\}$ and $k\in \{1,2\}$ such that $p_k(\lst P_i)\in \set G(a)$ and $p_{3-k}(\lst P_i)\in \set G(b)$.
\end{lemma}

\begin{proof}
If this were not the case, then every operator in $\set G(a)$ would commute with every operator in $\set G(b)$, and thus $[a,b]=0$, leading to a contradiction.
\end{proof}
%@nonl
%@-node:gcross.20100119121615.1280:Each pair must contain a top and a bottom
%@+node:gcross.20100119121615.1279:Restricted subset can be used to generate
\begin{lemma}
\label{restricted-subset-can-be-used-to-generate}
In the context of Proposition \ref{proposition-optimality-condition}, suppose we are given a pair of operators, $a,b\in(\genfun\circ\set U)(\lst P)$, such that $\{a,b\}=0$.  Let $k$ be the smallest index such that $m(\lst E_k)\ge M(a,b).$  Then $\set G\cap \{p_1(\lst P_i): i\ge k\}\ne\emptyset$.
\end{lemma}

\begin{proof}
By Lemma \ref{each-pair-must-contain-a-top-and-a-bottom}, we know that there must be some operator $o\in\{p_1(\lst P_i)\}_i$ such that $g\in\set G(a)\cup\set G(b)$.  By Lemma \ref{combinations-can't-make-things-better}, we know that all operators $g\in \set G(a)\cup \set G(b)$ must satisfy $\om_{\set S}(g)\ge m(\lst E_k)$.  Thus, we conclude that there must exist an operator $o\in\{p_1(\lst P_i): i\ge k\}$ such that $O\in \set G(a)\cup\set G(b)$, and hence $\set G\cap \{p_1(\lst P_i): i\ge k\}\ne\emptyset$.
\end{proof}
%@-node:gcross.20100119121615.1279:Restricted subset can be used to generate
...
We now have the tools that we need to prove the main proposition.

\begin{proof}[Proof of Proposition \ref{proposition-optimality-condition}]

Proof by contradiction.  Let $\lst P'$ be some choice of qubits stabilized by $\set S$ such that $(\genfun\circ\set U)(\lst P)=(\genfun\circ\set U)(\lst P')$ and there exists some integer $l$ such that $M(\lst P')_l > M(\lst P)_l$.  Let $\set C:=\{\lst P'_i : i \ge l\}$.  Let $m$ be the smallest integer such that $\om_{\set S}(\lst E_m)\ge M(\lst P')_l$, and $\set A := \{p_1(\lst P_i) : i \ge m\}$; note that since $M(\lst P')_l > M(\lst P)_l$ we must have $m>l$, and hence $|\set C| > |\set A|$.  By our construction of $\set A$ and Lemma \ref{restricted-subset-can-be-used-to-generate}, we know that for every pair $O\in\set C$ we must have $\set G(O)\cup\set A \ne\emptyset$.  Observe that this means that sets $\set C$ and $\set A$ match the descriptions in Lemma \ref{directed-gaussian-elimination-of-logicals}, and thus by Corolary \ref{death-stroke}, we see that it is impossible for $|\set C|>|\set A|$, and so we have a contradiction.  We thus conclude that there is no such choice $\lst P'$.
\end{proof}
%@-node:gcross.20091221145013.1285:Proof
%@-node:gcross.20091221145013.1280:Proposition: Certificate = Optimality
...
\item every member of every pair in $\set Q$ commutes with every operator in $\lst E$ and $\lst F$;
\item $m(X)\ge\om_{\set S}(o)$ for every $o\in\lst E$;
\item $|\lst E|+|\{\lst F_i : \lst F_i\ne I\}|=k$;
\end{itemize}
\end{proposition}
%@nonl
%@-node:gcross.20100119183304.1294:Statement
%@+node:gcross.20100119183304.1296:Lemma: Multiple recombinations
First, we need the following Lemma:

\begin{lemma}
Suppose we are given a pair $(a,b)$ and a seqence of $N$ pairs $\lst P$ all conjugal to some set $\set S$, and two sequences of elements from $\{0,1\}$, $\lst l^{(1)}$ and $\lst l^{(2)}$.  Define $b':=b\cdot\prod_{o\in \{p_{3-k}(\lst P_i) : k \in \{1,2\}, l^{(k)}=1\}}o$ and $\lst Q_i := (q^{(1)}_i,q^{(2)}_i)$ where
$$q^{(k)}_i :=
\begin{cases}
p_k(\lst P_i) & l^{(k)}_i = 0\\
p_k(\lst P_i) \cdot a & l^{(k)}_i = 1,
\end{cases}
$$
and
$$\set S' := \left[\set S\backslash\paren{\{(a,b)\} \cup \{\lst P_i\}_i)}\right]\cup \{(a,b')\} \cup \{\lst Q_i\}_i.$$
Then
\begin{enumerate}
\item the pair $(a,b')$ and the sequence of pairs $\lst Q$ are all conjugal pairs with respect to $\set S'$; and
\item $\genfun\paren{\{a,b'\}\cup \set U(\lst Q)}=\genfun\paren{\{a,b\}\cup \set U(\lst P)}$
\end{enumerate}
\end{lemma}
...
Now suppose we have proven this for all sequences of $N-1$ pairs, and we are given a sequence of $N$ pairs.  Invoking the inductive hypothesis, apply this Lemma recursively to the first $N-1$ elements of $\lst P$ and $\lst l$, and let $b''$, $\lst Q'$ and $\set S''$ refer to the objects described in the Lemma (without the extra primes) as applied to these $N-1$ elements.
...
\begin{cases}
b'' & l^{(1)}_N = 0\\
b'' \cdot p_2(\lst P_N)  &  l^{(1)}_N = 1,\\
\end{cases}
\quad
d_1 :=
\begin{cases}
p_1(\lst P_N) & l^{(1)}_N = 0\\
p_1(\lst P_N) \cdot a &  l^{(1)}_N = 1,\\
\end{cases}
$$
and
$$\set R := \left[\set S'\backslash\paren{\{(a,b''),\lst P_N\}}\right]\cup \{(a,c)\} \cup \{(d_1,p_2(\lst P_N)\}.$$
Note that for either value of $l^{(1)}$ we have that the pairs $(a,c)$ and $(d_1,p_2(\lst P_N))$ are conjugal with respect to $\set R$ and that $\genfun\paren{\{a,c,d_1,p_2(\lst P_N)\}}=\genfun\paren{\{a,b'',p_1(\lst P_N),p_2(\lst P_N)\}}$; for $l^{(1)}=0$ this conclusion is trivial since $b''=c$ and $d_1=p_1(\lst P_N)$, and for $l^{(1)}=1$ we obtain this conclusion by applying Lemma \ref{combining-pairs}.
...
d_2 :=
\begin{cases}
p_2(\lst P_N) & l^{(2)}_N = 0\\
p_2(\lst P_N) \cdot a &  l^{(2)}_N = 1,\\
\end{cases}
$$
and
$$\set R' := \left[\set R\backslash\paren{\{(a,c),(d_1,p_2(\lst P_N)\}}\right]\cup \{(a,c')\} \cup \{(d_1,d_2\}.$$
...
$$c'' :=
\begin{cases}
c' \cdot a & l^{(1)}_N = 1 \,\,\text{and}\,\, l^{(2)}_N = 1 \\
c' & \text{otherwise},\\
\end{cases}
$$
and
$$
\set R'':=
\left[\set R\backslash\paren{\{(a,c')\}}\right]\cup \{(a,c'')\}.
$$
Observe that $\{a,c''\}=0$ and that $a$ and $c''$ are products of $a$ and $c$;  thus, $a$ and $c''$ commute with every operator in $\set R''$ save each other and so $\{(a,c'')\} \cup \{(d_1,d_2\}$ are conjugal pairs with respect to $\set R''$, and also $\genfun\paren{\{a,c'',d_1,d_2\}}=\genfun\paren{\{a,c',d_1,d_2\}}$.
...
By recursive application of this Proposition, we obtain the objects (without the primes) $\lst P'$, $\set Q'$, $\lst E'$, and $\lst F'$.  Now consider the minimizer of $\om_{\set S}$ over $\set U(\set Q) \cup \{p_2(\lst P_i):\lst F_i \ne I\}$.  There are two cases:  either the minimizer is in $\set U(\set Q)$, or the minimizer is in $\{p_2(\lst P_i):\lst F_i \ne I\}$.

First we consider the first case.  
