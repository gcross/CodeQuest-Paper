...
such that
\begin{enumerate}
\item the pairs in $\lst P$ and $\set Q$ are all conjugal with respect to $\set R := \set U(\lst P)\cup \set U(\set Q)\cup \set S$;
\item $\genfun\paren{\set U(\lst P)\cup \set U(\set Q)}=(\genfun\circ\set U)(\set L)$;
\item every operator in $\set U(\set Q)$ commutes with every operator in $\{\lst O^{(l)}_i\}_{l,i}$;
\item $n \ge m_{\set S}(\lst P_{|\lst P|})$ if $\lst P'$ is non-empty;
\item $\sum_{l=1}^{n} |\lst O^{(l)}| = k$;
...
\item $\{\lst O^{(l)}_i\}_i=\{o:o\in\set U(\lst P), \om_{\set S}(o)=l\}$ for $l<m_{\set S}(\lst P_{|\lst P|})$;
\item for every $1\le l\le n$ and $1 \le j \le |\lst O^{(l)}|$ there exists an undetectable error (with respect to $\set S$) of weight $l$ that acts on $\lst O^{(l)}_j$ but not on any operator after $\lst O^{(l)}_j$, i.e. any operator in $\{\lst O^{(l)}_i : i > j\}$.
\end{enumerate}
\end{proposition}
...
\end{cases}
$$
(Note that $\lst O^{(l)}$ is the empty sequence for $d>n$ and $n < l < d$.)
...
\end{enumerate}
  ...
  First observe that  we know that the pair $(a,b')$ and the sequence of pairs $\lst Q'$ are all conjugal pairs with respect to $\set S'$, 
