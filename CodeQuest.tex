...
\end{enumerate}
\item a subset of the integers $1$ through $N$, $\set F\subseteq \{1\dots N\}$ such that if $i\ne \set F$ then $w\paren{e_{\set S} \paren{p_2(\lst P_i)}} \ge w(\lst E_N)$ --- that is, if $i$ is not a member of $\set F$ then the weight of the minimum weight undetectable error (with respect to $\set S$) on the second operator in $\lst P_i$ is no less than the weight of $\lst E_N$;
\item and maps $f:\set F\to \pauligroup$ and $g:\set F\to \{1\dots N\}$ such that for every $i\in \set F$,
\begin{enumerate}
\item $f(i)$ is an undetectable error (with respect to $\set S$) acting on $p_2(\lst P_i)$;
\item $w\paren{f(i)}=w\paren{e_{\set S}\paren{p_2(\lst P_i)}}$;
...
\item $f(i)$ commutes with every operator in $\{p_2(\lst P_j)\}_{j<i, j\notin \set F}$;
...
\begin{definition}
Within the context of Proposition \ref{proposition-optimality-condition}, we define the function $\set G:\genfun\paren{\set U(\lst P)}\to\powerset\paren{\set U(\lst P)}$ such that $\set G(o)$ is the \emph{generating set} of $o$, that is the set such that $o=\prod_{g\in\set G(o)} g$.
\end{definition}
%@nonl
%@-node:gcross.20091221145013.1286:Definition: Generating set
...
\begin{lemma}
In the context of Proposition \ref{proposition-optimality-condition}, if we are given an operator $o\in \genfun\paren{\set U(\lst P)}$ such that the set $\set G(o)$ contains at least one operator that is the first member of a pair in $\lst P$, then $o$ is a minimizer of $w\circ e_{\set S}$ over the set $\set G(o)$.
\end{lemma}

\begin{proof}[Proof of Lemma]
Let $c$ be the minimizer of $w \circ e_{\set S}$ over the set $\set G(o)$ such that the index of the pair $\lst P_i$ to which $c$ belongs is minimized, and if both operators in $\lst P_i$ are minimizers then let $c$ be the first one.  If we can show that $w\paren{e_{\set S}(o)} \le w\paren{e_{\set S}(c)}$.

Suppose that $c$ is the second member of the pair and $i\notin\set F$.  Since for every $i$ we have that $w(\lst E_i) \le w(\lst E_n) \le w\paren{e_{\set S}(c)}$ by the assumptions of the Proposition, due to our choice of $c$ this must mean that there is no operator in $\set G(o)$ which anti-commutes with any operator in the sequence $\lst E$, and hence that no operator in $\set G(o)$ is the first member of a pair in $\lst P$, which contradicts an assumption of this Lemma.  Hence, we conclude that either $c$ is the first member of the pair, or that $c$ is the second member of the pair and $i\in\set F$.

Take any operator $x\in \set G(o)$ that anti-commutes with $e_{\set S}(c)$, and let $\lst P_j$ be the pair of which it is a member.  Since $x$ must satisfy $w\paren{ e_{\set S}(x)}\le w\paren{ e_{\set S}(c)}$, we observe that due to our choice of $c$ we cannot have $j < i$.  Furthermore, observe that by our definition of $c$, the conclusion of the previous paragraph, and the assumptions of the Proposition, we cannot have $j > i$ since every operator in a pair $\lst P_k$ such that $k > i$ must commute with $e_{\set S}(c)$.  Thus, we conclude that $i=j$, and due to our choice of $c$ it must furthermore be the case that $c=x$, which means that $c$ is the only operator in $\set C$ that anti-commutes with $e_{\set S}(c)$.  Thus, $o$ must also anti-commute with $e_{\set S}(c)$, and so $w\paren{e_{\set S}(o)} \le w\paren{e_{\set S}(c)}$. \qed
\end{proof}
%@-node:gcross.20091221145013.1284:Combinations cannot make things better
%@+node:gcross.20091221145013.1287:Gaussian elimination like procedure
\begin{lemma}
Suppose that within the context of Proposition \ref{proposition-optimality-condition}, we are given
\begin{itemize}
\item a sequence, $\lst A$, of $k$ independent, commuting elements of $\set U(\lst P)$,
\item a set, $\set B$, of commuting elements of $\set U(\lst P)$, such that for every $i\in\{1\dots k\}$ there exists a $b\in \set B$ such that $\{\lst A_i,b\}=0$;
\item a sequence, $\lst Q$, of $k$ conjugal pairs with respect to $\set S$, such that
\begin{itemize}
\item $\set U(\lst Q)\subseteq\genfun\paren{\{\lst A_i\}_i \cup \set B};$
\item the function $m_{\set S}$ is constant over the set $\{\lst Q_i\}_i$; and
\item $\set G(g)\cap \{\lst O_i\}_i\ne \emptyset$ for every operator $g\in\{p_1(\lst P_i)\}_i$.
\end{itemize}
\end{itemize}
Then there exists a sequence $\set Q'$ of $k$ conjugal pairs with respect to $\set S$ such that
\begin{itemize}
\item $\genfun\paren{\set U(\lst Q')}=\genfun\paren{\set U(\lst Q)}$;
\item $m(\lst Q'_i)=m(\lst Q_i)$ for all $i\in\{1\dots k\}$; and
\item $\set G\paren{p_1(\lst Q'_i)}=\{\lst O_i\}$ for all $i\in\{1\dots k\}$.
\end{itemize}
\end{lemma}

\begin{proof}
Proof by induction.  Note that the statement of the Lemma holds trivially for $k=1$ by setting $\lst Q':=\lst Q$.

Now suppose we have been given sequences of length $k$.  By the inductive hypothesis, we can find a sequence length $k-1$, $\lst P''$, that satisfies the properties of the Lemma for the first $k-1$ elements of $\lst O$ and $\lst Q$.  Let
$$q := \lst Q_N \cdot \prod_{\lst O_i \in \set G(\lst Q_N)} \lst Q''_i.$$
Note that by construction, $\set G(q)\cap \{\lst O_i\}_i \subseteq \{\lst O_N\}$, since any other element from $\lst O$ appears in either no factor in the product above or in exactly two.  Furthermore, 
\end{proof}
%@-node:gcross.20091221145013.1287:Gaussian elimination like procedure
%@-node:gcross.20091221145013.1282:Lemmas
