...
\item the function $u:\powerset(\pauligroup\times\pauligroup)\to\powerset(\pauligroup)$ is defined (for convenience) such that $u\paren{\set X}:=\bigcup_{(a,b)\in\set X} \{a,b\}$ --- that is, it `unpacks' all of the conjugal pairs into a set of operators; and finally,
\item an \emph{optimal choice of logical qubits} is any ordered sequence $\lst P$ conjugal pairs such that
\begin{enumerate}
\item $u\paren{\lst P}$ satisfies the properties of $\set L$ (given $\set S$ and $\set G$) in Theorem \ref{theorem-SG}, and \\
