...
Let $\set D:=\{X\}\cup\set D'$.  Since $(\genfun\circ\set U)(\{Y\}\cup\set D')=(\genfun\circ\set U)(\set C')$ and by Lemma \ref{directed-gaussian-elimination-of-logicals} we know that $X$ is a conjugal pair with respect to $\{X\}\cup\set D'\cup\set S$, which implies that $|\set D|=|\set D'|+1=|\set C|-1$ and thus $|\{Y\}\cup\set D|=|\set C|$, and we also that $(\genfun\circ\set U)(\{Y\}\cup\set D)=(\genfun\circ\set U)(\set C)$.  By a combination of the recursive application of this Corolary we know that $\{p_1(O):O\in\set B\backslash\{(c,d)\}\}\cap p_1(Y)=\{a\}$, $\{p_1(O):O\in\set B\backslash\{(c,d)\}\}\cap p_2(Y)=\emptyset$ $b\in p_2(Y)$ and $b\notin p_1(Y)$.  By Lemma \ref{directed-gaussian-elimination-of-logicals} we know that $c\notin \bigcup_{b\in(\genfun\circ\set U)(\set B)} \set G(b)$, and thus $\{p_1(O):O\in\set B\}\cap p_1(Y)=\{a\}$ and $\{p_1(O):O\in\set B\}\cap p_2(Y)=\emptyset$.
\end{proof}
%@-node:gcross.20100101105725.1298:Elimination to isolate
%@-node:gcross.20091226225139.1291:Directed Gaussian elimination
%@+node:gcross.20100101105725.1299:Sequence rearrangement
\begin{lemma}
In the context of Proposition \ref{proposition-optimality-condition}, suppose we have a conjugal pair with respect to $\set S$, $(a,b):=O\in(\genfun\circ\set U)(\lst P)$, such that
\begin{itemize}
\item $m(O)=m(\lst P_n)$;
\item $\{p_1(\lst P_i)\}_i\cup (\set G\circ p_1)(O)=\{p_1(\lst P_n)\}$; 
\item $p_2(\lst P_n)\notin (\set G\circ p_1)(O)$; and
\item $p_2(\lst P_n)\in (\set G\circ p_2)(O)$.
\end{itemize}
Then there exists a choice of qubits with respect to $\set S$, $\lst P'$, such that
\begin{itemize}
\item $\lst P_N=O$;
\item $(\genfun\circ\set U)(\lst P)=(\genfun\circ\set U)(\lst P')$;
\item $\lst P'$ satisfies the same properties listed in Proposition \ref{proposition-optimality-condition} as $\lst P$;
\item $\lst M^{(N)}(\lst P')=\lst M^{(N)}(\lst P)$;
\end{lemma}
