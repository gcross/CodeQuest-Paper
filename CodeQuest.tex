...
\item \label{nothing smaller in Q} if $\lst P$ is non-empty, then $n \ge m_{\set S}(\lst P_{|\lst P|})$ and $\om_{\set S}(o)\ge m_{\set S}(\lst P_{|\lst P|})$ for all $o\in\set U(\set Q)$;
\item $\left|\{\lst O^{(l)\}_{l,i}\right| = k$;
...
\item for every $1\le l\le n$ and $1 \le j \le |\lst O^{(l)}|$ there exists an undetectable error (with respect to $\set S$) of weight $l$ that acts on $\lst O^{(l)}_j$ but not on any operator in $\{\lst O^{(l)}_i : i > j\} \cup \set U(\set Q)$.
\end{enumerate}
\end{proposition}
