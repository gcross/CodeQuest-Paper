...
\begin{lemma}
Suppose we are given a pair $(a,b)$ conjugal to some set $\set S$, and a set of conjugal pairs $\set O$.  Let $b':=b\cdot \prod_{o\in\set O} o)$ and $O':=\{o\cdot a: o\in\set O}$.  Then the pair $(a,b')$ is conjugal to $(\set S\backslash\{b\})\cup\{b'\}$, and $\genfun(\{a,b'\}\cup\set O'=\genfun(\{a,b\}\cup\set O$.
\end{lemma}

\begin{proof}
Proof by induction.  For the base case, note that statement trivially holds for $\set O=\emptyset$.

Now suppose we have proven this for $|\set O|=N-1$, and we are given a $\set O$ such that $|\set O|=N$.  Take any element $o\in\set O$.   and let $\set P:=\set O\backslash\{o\}$.
%@nonl
%@-node:gcross.20100119183304.1296:Lemma: Multiple recombinations
