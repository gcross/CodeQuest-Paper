...
\begin{proposition}
Suppose that we have been given lists of Pauli operators $\bar I$ and non-empty $\bar D$ acting on $N$ qubits as well as an integer $0\le k\le N$ such that for all integers $1 \le i \le k$, the following properties hold:
\begin{enumerate}
\item in each ($N-$qubit tensor product) operator of $\bar D$, the single-qubit operator acting on qubit $i$ is the identity;
\item there is at most one operator in $\bar I$ such that the $i$-th sub-operator is $X$;
\item there is at most one operator in $\bar I$ such that the $i$-th sub-operator is $Z$.
\end{enumerate}
...
\begin{proof}
Define $\bar D_j\subseteq \bar D$ to be the sub-list of operators in $\bar D$ such that the $j$-th sub-operator is not the identity, and take $k'$ to be the smallest integer strictly larger that $\bar D_{k'}$ is non-empty.  
\end{proof}
